...
class Pro_Ads_Init {	
	...
	*/
	public function wppas_enqueue_scripts()
	{
		...
		wp_register_style( 'wp_pro_add_style', WP_ADS_TPL_URL."/css/wpproadds.min.css", false, WP_ADS_VERSION, "all" );
		...
		if( $pro_ads_multisite->pro_ads_load_admin_data() )
		{
			...
			if(is_admin() && isset( $_GET['page'] ) || is_admin() && isset($_GET['post_type']) || is_admin() && isset($_GET['post']) && $this->wp_pro_ad_check_cpt( get_post_type( $_GET['post'] )) )
			{
				if( 
					...
					// Main CSS
					wp_enqueue_style("wp_pro_ads_main_style", WP_ADS_TPL_URL."/css/wpproadds.min.css", false, WP_ADS_VERSION, "all");
	...
	*/
	...
	{	
		...
		//register_widget('Pro_Ads_Ad_Slot');
	}
