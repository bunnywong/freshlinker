					...
					if ($logo_display == "custom_image") {?>
					    <a class="logo-img" href="<?php echo esc_url(home_url('/'));?>" itemprop="url" title="<?php echo esc_attr(get_bloginfo('name','display'))?>">
					    	<?php if (isset($logo_img) && $logo_img != "") {?>
					    		<img class="default_logo" itemprop="logo" alt="" src="<?php echo $logo_img?>">
					    	<?php }
					    	if (isset($retina_logo) && $retina_logo != "") {?>
					    		<img class="retina_logo" itemprop="logo" alt="" src="<?php echo $retina_logo?>">
					    	<?php }?>
					    </a>
					<?php }else {?>
									...
									<?php if ($remove_index_content != 1) {
										if ($index_top_box_layout == 2) {?>
											<div class="col-md-12">
												...
												<form class="form-style form-style-2">
													<p>
														<input type="text" id="question_title" value="<?php echo $ask_a_new_question;?>" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;">
										...
										<?php }else {?>
											...
											<div class="col-md-9">
												<form class="form-style form-style-2">
													<p>
														<textarea rows="4" id="question_title" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;"><?php echo $ask_a_new_question;?></textarea>
														<i class="icon-pencil"></i>
														<span class="color button small publish-question"><?php _e("Ask Now","vbegy");?></span>
													</p>
												</form>
						...
						if (isset($if_user_id->caps["activation"]) && $if_user_id->caps["activation"] == 1) {
							...
							if (isset($_GET['u']) && isset($_GET['activate'])) {
								...
								if ($activation == $get_activate) {
									...
									echo '<div class="alert-message error"><i class="icon-ok"></i><p><span>'.sprintf(__("Activate the membership","vbegy").'</span><br>'.__("Please activate your membership, Click on the link has been sent to email <a href='%s'>Click here</a> to send it again .","vbegy"),esc_url(add_query_arg(array("get_activate" => "do"),esc_url(home_url('/'))))).'</p></div>';
								}
								...
								if (isset($_GET['get_activate']) && $_GET['get_activate'] == "do") {
									...
									<a href=".esc_url(add_query_arg(array("u" => get_current_user_id(),"activate" => $rand_a),esc_url(home_url('/')))).">".__("Activate","vbegy")."</a><br />
