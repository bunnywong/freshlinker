...
if ($yes_private == 1) {?>
	<article <?php post_class('question'.$question_type);?> id="post-<?php the_ID();?>" role="article" itemscope="" itemtype="http://schema.org/Article">
		...
		<div class="question-author">
			...
			<?php }?>
				...
				if (get_the_author_meta('you_avatar', get_the_author_meta('ID'))) {
					...
					if ($post->post_author != 0) {
						echo get_avatar(get_the_author_meta('user_email',get_the_author_meta('ID')),'65','');
					}else {
		...
		<div class="question-inner">
			...
			<?php }?>
			<span class="question-date" datetime="<?php the_time('c'); ?>" itemprop="datePublished"><i class="icon-time"></i><?php echo human_time_diff(get_the_time('U'), current_time('timestamp'));?></span>
			...
			<span class="question-view"><i class="icon-user"></i><?php $post_stats = get_post_meta($post->ID, 'post_stats', true);echo ($post_stats != ""?$post_stats:0);?> <?php _e("views","vbegy");?></span>
