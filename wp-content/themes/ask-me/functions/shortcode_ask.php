						...
						<form class="new-question-form" method="post" enctype="multipart/form-data">
							...
							<div class="form-inputs clearfix">';
								...
								$out .= '<p>
									...
									<input name="title" id="question-title-'.$rand_q.'" class="the-title" type="text" value="'.(isset($posted['title'])?$posted['title']:'').'">
...
function is_user_logged_in_data ($user_links) {
	$out = '';
	if (is_user_logged_in()) {
							...
							$out .= '<div class="col-md-6">
								<ul class="user_quick_links">';
									...
									if (isset($user_links) && is_array($user_links) && (isset($user_links["answers"]) && ($user_links["answers"] == 1 || $user_links["answers"] == "on"))) {
										$out .= '<li><a href="'.esc_url(add_query_arg(array_merge(array("u" => esc_attr($user_login->ID),$get_lang_array)),get_page_link(vpanel_options('answer_user_page')))).'"><i class="icon-comment"></i>'.__("Answers","vbegy").'</a></li>';
									}
							...
							$out .= '<div class="col-md-6">
								<ul class="user_quick_links">';
									...
									if (isset($user_links) && is_array($user_links) && (isset($user_links["follow_answers"]) && ($user_links["follow_answers"] == 1 || $user_links["follow_answers"] == "on"))) {
										$out .= '<li><a href="'.esc_url(add_query_arg(array_merge(array("u" => esc_attr($user_login->ID),$get_lang_array)),get_page_link(vpanel_options('follow_answer_page')))).'"><i class="icon-comment"></i>'.__("Follow answers","vbegy").'</a></li>';
									}
									...
									if (isset($user_links) && is_array($user_links) && (isset($user_links["follow_comments"]) && ($user_links["follow_comments"] == 1 || $user_links["follow_comments"] == "on"))) {
										$out .= '<li><a href="'.esc_url(add_query_arg(array_merge(array("u" => esc_attr($user_login->ID),$get_lang_array)),get_page_link(vpanel_options('follow_comment_page')))).'"><i class="icon-comments"></i>'.__("Follow comments","vbegy").'</a></li>';
									}
									...
									if (isset($user_links) && is_array($user_links) && (isset($user_links["logout"]) && ($user_links["logout"] == 1 || $user_links["logout"] == "on"))) {
										$out .= '<li><a href="'.wp_logout_url(home_url()).'"><i class="icon-signout"></i>'.__("Logout","vbegy").'</a></li>';
									}
								$out .= '</ul>
							</div>';
						}
					...
					if ($the_captcha_register == 1) {
						if ($captcha_style == "question_answer") {
							...
							<p class='ask_captcha_p'>
								...
								<input size='10' id='ask_captcha-".$rand_w."' name='ask_captcha' class='ask_captcha' value='' type='text'>
								<span class='question_poll ask_captcha_span'>".$captcha_question.($show_captcha_answer == 1?" ( ".$captcha_answer." )":"")."</span>
							</p>";
						}else {
							$out .= "
