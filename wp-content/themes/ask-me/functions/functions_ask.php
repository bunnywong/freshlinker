...
function report_q () {
	...
	sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),vpanel_options("email_template"),get_bloginfo('name'),__("Hi there","vbegy"),$last_message_email);
	die();
}
...
function comment_question($comment_id) {
	global $post,$vpanel_emails,$vpanel_emails_2,$vpanel_emails_3;
	if ($post->post_type == "question") {
		...
		if ($remember_answer == 1 && $post->post_author != $user_id) {
			...
			if ($post->post_author != 0) {
				...
				$the_mail = get_post_meta($post->ID, 'question_email',true);
				$the_author = get_post_meta($post->ID, 'question_username',true);
			}
			...
			$last_message_email = $vpanel_emails.'<img src="'.vpanel_options("logo_email_template").'" alt="">'.$vpanel_emails_2.$post_mail.$vpanel_emails_3;
			sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),$the_mail,$the_author,__("Hi there","vbegy"),$last_message_email);
		}
		...
		if ($question_follow == 1 && isset($following_questions[0]) && is_array($following_questions[0])) {
			...
			<a href=".get_permalink($post->ID)."#comment-".$get_comment->comment_ID.">".$post->post_title."</a><br />";
			
			$last_message_email = $vpanel_emails.'<img src="'.vpanel_options("logo_email_template").'" alt="">'.$vpanel_emails_2.$post_mail.$vpanel_emails_3;
			foreach ( $following_questions[0] as $user ) {
				$all_meta_for_user = get_user_by('id',$user);
				sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),esc_html($all_meta_for_user->user_email),esc_html($all_meta_for_user->display_name),__("Hi there","vbegy"),$answer_mail);
			}
		}
		...
		if ($user_id != 0) {
			$user_vote = get_user_by("id",$user_id);
			if ($user_id != $post->post_author && $active_points == 1) {
				$_points = get_user_meta($user_id,$user_vote->user_login."_points",true);
				$_points++;
				
				update_user_meta($user_id,$user_vote->user_login."_points",$_points);
				add_user_meta($user_id,$user_vote->user_login."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),(vpanel_options("point_add_comment") != ""?vpanel_options("point_add_comment"):2),"+","answer_question",$post->ID,$get_comment->comment_ID));
						...
						if ($yes_private == 1) {
							...
							foreach ( $users as $user ) {
								if (is_array($send_email_question_groups) && in_array($user->roles[0],$send_email_question_groups)) {
									sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),esc_html($user->user_email),esc_html($user->display_name),__("Hi there","vbegy"),$last_message_email);
								}
							}
						}
					}
...
/* vbegy_session_post */
function vbegy_session_post ($message = ""){
	$post_publish = vpanel_options("post_publish");
	if ($post_publish == "draft") {
		if(!session_id())
			session_start();
		if ($message) {
			$_SESSION['vbegy_session_post'] = $message;
		}else {
			if (isset($_SESSION['vbegy_session_post'])) {
				$last_message = $_SESSION['vbegy_session_post'];
				unset($_SESSION['vbegy_session_post']);
				echo $last_message;
			}
		}
	}
...
function ask_process_edit_profile_form() {
	...
	if ( is_wp_error( $errors_user ) ) return $errors;
