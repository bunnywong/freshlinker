...
<div id="page-title-wrapper">
	<div class="container">
		...
		<?php 
			$cp_ldin_uri = get_post_meta( get_the_ID(), 'cp_ldin_address', true );
			$cp_ldin_user = get_post_meta( get_the_ID(), 'cp_ldin_user', true );
			$cp_person = get_post_meta( get_the_ID(), 'cp_person', true );
			$cp_person_title = get_post_meta( get_the_ID(), 'cp_person_title', true );
			$cp_email = get_post_meta( get_the_ID(), 'cp_email', true );
			$cp_phone = get_post_meta( get_the_ID(), 'cp_phone', true );
			if ( $cp_ldin_uri || $cp_ldin_user || $cp_person || $cp_person_title || $cp_email || $cp_phone ) : ?>
		
				<div class="abount-company-content cperson">
					<h2 class="sub-section-title uppercase"><?php echo esc_attr( __( 'Contact Person', 'jobboard' ) ); ?></h2>
					<?php if ( $cp_ldin_user ) { ?>
						<span>
							<?php if ( $cp_ldin_uri ) echo '<a href="' . esc_url( $cp_ldin_uri ) . '">'; ?>
							<?php echo esc_attr( $cp_ldin_user ); ?>
							<?php if ( $cp_ldin_uri ) echo '</a>'; ?>
						</span>
					<?php } ?>
					<?php if ( $cp_person ) { ?>
						<span>
							<?php if ( $cp_person_title ) echo esc_attr( $cp_person_title ) . '. '; ?>
							<?php echo esc_attr( $cp_person ); ?>
						</span>
					<?php } ?>
					<?php if ( $cp_email ) { ?>
						<span>
