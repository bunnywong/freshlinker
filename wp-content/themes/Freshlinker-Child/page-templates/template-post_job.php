	...
	<div class="container">
		...
		<?php
			...
			if( isset( $_GET['action'] ) && $_GET['action'] == 'edit' ){
				...
				$job_nature = wp_get_post_terms( $edit->ID, 'job_nature' );
		...
		<div class="row">
			...
			<div class="col-md-12">
				...
				<form id="post-job-form" class="frontend-form" action="" method="post" entype="multipart/form-data" role="form">
					...
					<div class="form-group">
						<label for="job_nature"><?php _e( 'Job Nature', 'jobboard' ); ?></label>
						<select name="job_nature" id="job_nature" class="form-control">
							<option value=""><?php echo '-- '.__( 'Select Job Nature', 'jobboard' ).' --'; ?></option>
							<?php 
								$terms = get_terms( 'job_nature', array( 'hide_empty' => false, ) );
								foreach( $terms as $term ){
									$selected = '';
									if( $default['job_nature'] == $term->slug ){
										$selected = 'selected="selected"';
									}
									echo '<option value="'.$term->slug.'" '.$selected.'>'.esc_attr($term->name).'</option>';
								}
					...
					<div class="form-group">
						...
						<label for="job_sallary"><?php _e( 'Salary', 'jobboard' ); ?></label>
					...
					<div class="form-group">
						...
						<span class="form-desc"><?php _e( 'Attract relevant job seekers to read further. (Maximum 100 characters)', 'jobboard' ); ?></span>

						<input type="text" name="job_summary" id="job_summary" class="form-control" maxlength="100" value="<?php echo esc_attr( $default['job_summary'] ); ?>" />
