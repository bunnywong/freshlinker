...
function jobboard_setup() {
  ...
  add_image_size( 'jobboard-full-size-image', 9999, 9999, true); // Full size
  add_image_size( 'jobboard-company-logo', 300, 99999, false); // Company image size
  add_image_size( 'jobboard-company-testimonial-thumbnail', 100, 100, true ); // Company Testimonial Avatar Thumbnail
  add_image_size( 'jobboard-company-portfolio', 370, 206, true ); // Company Testimonial Avatar Thumbnail
  add_image_size( 'jobboard-resume-listing-img', 52, 52, true ); // Resume listing avatar
  ...
  add_theme_support( 'title-tag' );
	...
	if( !function_exists( 'jobboard_custom_title' ) ){
		add_filter( 'wp_title', 'jobboard_custom_title', 10, 2 );
		function jobboard_custom_title( $title, $sep )
		{
      if ( is_feed() ) {
  			return $title;
  		}
  		...
  		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
  			$title .= " $sep " . sprintf( __( 'Page %s', 'jobboard' ), max( $paged, $page ) );
  		}
