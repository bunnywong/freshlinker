<?php get_header();
	if ( have_posts() ) : while ( have_posts() ) : the_post();
		...
		$vbegy_sidebar = $vbegy_sidebar_all = rwmb_meta('vbegy_sidebar','select',$post->ID);
		...
		<?php }else {
			...
			<article <?php post_class('question single-question'.$question_type);?> id="post-<?php the_ID();?>" role="article" itemscope="" itemtype="http://schema.org/Article">
				...
				if (($post->post_author != 0 && $user_login_id_l->ID == $user_get_current_user_id) || ($question_follow == 1 && is_user_logged_in()) || (($post->post_author != 0 && $user_login_id_l->ID == $user_get_current_user_id) || is_super_admin($user_get_current_user_id))) {?>
					<div class="edit-delete-follow-close">
						<h2>
							<?php if ($post->post_author != 0 && $user_login_id_l->ID == $user_get_current_user_id) {?>
								...
								<?php $question_delete = vpanel_options("question_delete");
				...
				<div class="question-inner">
					...
					<div class="question-desc" itemprop="mainContentOfPage">
						...
						if ($custom_permission != 1 || (is_user_logged_in() && isset($roles["show_question"]) && $roles["show_question"] == 1) || (!is_user_logged_in() && $show_question == 1)) {
							if ($active_reports == 1) {?>
							...
							//global $allowedtags;
							//echo wp_kses(get_the_content(),$allowedtags)."<div class='clearfix'></div><br>";
							echo the_content();
	...
	if ($yes_private != 1) {
		...
		if (isset($vbegy_custom_sections) && $vbegy_custom_sections == 1) {
			...
			if (empty($order_sections_li)) {
				$order_sections_li = array(1 => "advertising",2 => "author",3 => "related",4 => "comments",5 => "next_previous");
			}
						...
						if ($post->post_author != 0) {?>
							<div class="about-author clearfix">
							    <div class="author-image">
							    	<a href="<?php echo vpanel_get_user_url($authordata->ID,$authordata->user_nicename);?>" original-title="<?php the_author();?>" class="tooltip-n">
							    		<?php 
