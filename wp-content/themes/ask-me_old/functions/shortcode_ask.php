...
function ask_question_shortcode($atts, $content = null) {
	...
	if (is_user_logged_in()) {
		...
		//echo $user_login_group."<pre>";print_r($roles);echo "</pre>";
	}
	$settings = array("quicktags" => false,"teeny" => true,"textarea_name" => "comment","media_buttons" => false,"textarea_rows" => 7,'tinymce'=> array("toolbar1" => "bold,italic,underline,bullist,numlist,link,unlink,forecolor,undo,redo'"));
						...
						<form class="new-question-form" method="post" enctype="multipart/form-data">
							...
							<div class="form-inputs clearfix">';
								if (!is_user_logged_in() && $ask_question_no_register == 1) {
									$out .= '<p>
										<label for="question-username" class="required">'.__("Username","vbegy").'<span>*</span></label>
										<input name="username" id="question-username" class="the-username" type="text" value="'.(isset($posted['username'])?$posted['title']:'').'">
										<span class="form-description">'.__("Please type your username .","vbegy").'</span>
									</p>
									
									<p>
										<label for="question-email" class="required">'.__("E-Mail","vbegy").'<span>*</span></label>
										<input name="email" id="question-email" class="the-email" type="text" value="'.(isset($posted['email'])?$posted['title']:'').'">
										<span class="form-description">'.__("Please type your E-Mail .","vbegy").'</span>
									</p>';
								}
								$out .= '<p>
									<label for="question-title" class="required">'.__("Question Title","vbegy").'<span>*</span></label>
									<input name="title" id="question-title" class="the-title" type="text" value="'.(isset($posted['title'])?$posted['title']:'').'">
									<span class="form-description">'.__("Please choose an appropriate title for the question to answer it even easier .","vbegy").'</span>
								</p>';
								
								if ($tags_question == 1) {
									$out .= '<p>
										<label for="question_tags">'.__("Tags","vbegy").'</label>
										<input type="text" class="input question_tags" name="question_tags" id="question_tags" value="'.(isset($posted['question_tags'])?$posted['question_tags']:'').'" data-seperator=",">
								...
								$out .= '<p>
									<label for="question-category" class="required">'.__("Category","vbegy").'<span>*</span></label>
									<span class="styled-select">'.
										wp_dropdown_categories(array("orderby" => "NAME","echo" => "0","show_option_none" => __("Select a Category","vbegy"),'taxonomy' => 'question-category', 'hide_empty' => 0,'depth' => 0,'id' => 'question-category','name' => 'category',  'hierarchical' => true,'selected' => (int)$posted['category']))
									.'</span>
								...
								if ($poll_question == 1) {
									$out .= '<p class="question_poll_p">
										<label for="question_poll">'.__("Poll","vbegy").'</label>
										<input type="checkbox" id="question_poll" class="question_poll" value="1" name="question_poll" '.(isset($posted['question_poll']) && $posted['question_poll'] == 1?"checked='checked'":"").'>
							...
							<div>
								<label for="question-details" '.(vpanel_options("comment_question") == 1?'class="required"':'').'>'.__("Details","vbegy").(vpanel_options("comment_question") == 1?'<span>*</span>':'').'</label>';
								
								//wp_editor((isset($posted['comment'])?$posted['comment']:""),"question-details",$settings);
								
								$out .= '<textarea name="comment" id="question-details" class="the-textarea" aria-required="true" cols="58" rows="8">'.(isset($posted['comment'])?$posted['comment']:"").'</textarea>';
								
								$out .= '<div class="the-details the-textarea"></div>
								<p><span class="form-description">'.__("Type the description thoroughly and in details .","vbegy").'<br>
				...
				<div '.(!is_user_logged_in()?"class='if_no_login'":"").'>';
					...
					<form class="new-question-form" method="post" enctype="multipart/form-data">
						...
						<div class="form-inputs clearfix">
							<p>
								<label for="question-title" class="required">'.__("Question Title","vbegy").'<span>*</span></label>
								<input name="title" id="question-title" class="the-title" type="text" value="'.(isset($posted['title'])?$posted['title']:$get_post_q->post_title).'">
								<span class="form-description">'.__("Please choose an appropriate title for the question to answer it even easier .","vbegy").'</span>
							</p>';
							...
							if ($tags_question == 1) {
								$out .= '<p>
									<label for="question_tags">'.__("Tags","vbegy").'</label>
									<input type="text" class="input question_tags" name="question_tags" id="question_tags" value="'.(isset($posted['question_tags'])?$posted['question_tags']:$q_tag).'" data-seperator=",">
									<span class="form-description">'.__("Please choose  suitable Keywords Ex : ","vbegy").'<span class="color">'.__("question , poll","vbegy").'</span> .</span>
								</p>';
							}
							...
							$out .= '<p>
								<label for="question-category" class="required">'.__("Category","vbegy").'<span>*</span></label>
								<span class="styled-select">'.
									wp_dropdown_categories(array("orderby" => "NAME","echo" => "0","show_option_none" => __("Select a Category","vbegy"),'taxonomy' => 'question-category', 'hide_empty' => 0,'depth' => 0,'id' => 'question-category','name' => 'category',  'hierarchical' => true,'selected' => (isset($posted['category'])?(int)$posted['category']:$question_category)))
								.'</span>
								<span class="form-description">'.__("Please choose the appropriate section so easily search for your question .","vbegy").'</span>
							</p>';
						...
						<div>
							...
							$out .= '<textarea name="comment" id="question-details" class="the-textarea" aria-required="true" cols="58" rows="8">'.(isset($posted['comment'])?$posted['comment']:$get_post_q->post_content).'</textarea>';
...
function add_post_shortcode($atts, $content = null) {
	...
	if (($custom_permission == 1 && is_user_logged_in() && empty($roles["add_post"])) || ($custom_permission == 1 && !is_user_logged_in() && $add_post != 1)) {
		...
		$out .= '<div class="form-style form-style-3"><div class="note_error"><strong>'.__("You must login to add post .","vbegy").'</strong></div>'.do_shortcode("[ask_login]").'<ul class="login-links login-links-r"><li><a href="#">'.__("Register","vbegy").'</a></li></ul></div>';
	}else {
						...
						<form class="new-post-form" method="post" enctype="multipart/form-data">
							...
							<div class="form-inputs clearfix">';
								if (!is_user_logged_in() && $add_post_no_register == 1) {
									$out .= '<p>
										<label for="post-username" class="required">'.__("Username","vbegy").'<span>*</span></label>
										<input name="username" id="post-username" class="the-username" type="text" value="'.(isset($posted['username'])?$posted['title']:'').'">
										<span class="form-description">'.__("Please type your username .","vbegy").'</span>
									</p>
									
									<p>
										<label for="post-email" class="required">'.__("E-Mail","vbegy").'<span>*</span></label>
										<input name="email" id="post-email" class="the-email" type="text" value="'.(isset($posted['email'])?$posted['title']:'').'">
										<span class="form-description">'.__("Please type your E-Mail .","vbegy").'</span>
									</p>';
								}
								$out .= '<p>
									<label for="post-title" class="required">'.__("Post Title","vbegy").'<span>*</span></label>
									<input name="title" id="post-title" class="the-title" type="text" value="'.(isset($posted['title'])?$posted['title']:'').'">
									<span class="form-description">'.__("Please choose an appropriate title for the post .","vbegy").'</span>
								</p>';
								
								if ($tags_post == 1) {
									$out .= '<p>
										<label for="post_tag">'.__("Tags","vbegy").'</label>
										<input type="text" class="input post_tag" name="post_tag" id="post_tag" value="'.(isset($posted['post_tag'])?$posted['post_tag']:'').'" data-seperator=",">
										<span class="form-description">'.__("Please choose  suitable Keywords Ex : ","vbegy").'<span class="color">'.__("post , video","vbegy").'</span> .</span>
									</p>';
								}
								
								$out .= '<p>
									<label for="post-category" class="required">'.__("Category","vbegy").'<span>*</span></label>
									<span class="styled-select">'.
										wp_dropdown_categories(array("orderby" => "NAME","echo" => "0","show_option_none" => __("Select a Category","vbegy"),'taxonomy' => 'category', 'hide_empty' => 0,'depth' => 0,'id' => 'post-category','name' => 'category',  'hierarchical' => true,'selected' => (int)$posted['category']))
									.'</span>
							...
							<div>
								<label for="post-details" '.(vpanel_options("content_post") == 1?'class="required"':'').'>'.__("Details","vbegy").(vpanel_options("content_post") == 1?'<span>*</span>':'').'</label>';
								
								//wp_editor((isset($posted['comment'])?$posted['comment']:""),"post-details",$settings);
								
								$out .= '<textarea name="comment" id="post-details" class="the-textarea" aria-required="true" cols="58" rows="8">'.(isset($posted['comment'])?$posted['comment']:"").'</textarea>';
				...
				<div '.(!is_user_logged_in()?"class='if_no_login'":"").'>';
					...
					<form class="new-post-form" method="post" enctype="multipart/form-data">
						...
						<div class="form-inputs clearfix">
							<p>
								<label for="post-title" class="required">'.__("post Title","vbegy").'<span>*</span></label>
								<input name="title" id="post-title" class="the-title" type="text" value="'.(isset($posted['title'])?$posted['title']:$get_post_q->post_title).'">
								<span class="form-description">'.__("Please choose an appropriate title for the post .","vbegy").'</span>
							</p>';
							...
							if ($tags_post == 1) {
								$out .= '<p>
									<label for="post_tag">'.__("Tags","vbegy").'</label>
									<input type="text" class="input post_tag" name="post_tag" id="post_tag" value="'.(isset($posted['post_tag'])?$posted['post_tag']:$q_tag).'" data-seperator=",">
									<span class="form-description">'.__("Please choose  suitable Keywords Ex : ","vbegy").'<span class="color">'.__("post , video","vbegy").'</span> .</span>
								</p>';
							}
							...
							$out .= '<p>
								<label for="post-category" class="required">'.__("Category","vbegy").'<span>*</span></label>
								<span class="styled-select">'.
									wp_dropdown_categories(array("orderby" => "NAME","echo" => "0","show_option_none" => __("Select a Category","vbegy"),'taxonomy' => 'category', 'hide_empty' => 0,'depth' => 0,'id' => 'post-category','name' => 'category',  'hierarchical' => true,'selected' => (isset($posted['category'])?(int)$posted['category']:$category)))
								.'</span>
								<span class="form-description">'.__("Please choose the appropriate section so easily search for your post .","vbegy").'</span>
							</p>';
						...
						<div>
							...
							$out .= '<textarea name="comment" id="post-details" class="the-textarea" aria-required="true" cols="58" rows="8">'.(isset($posted['comment'])?$posted['comment']:$get_post_q->post_content).'</textarea>';
				...
				<div class="row">
					<div class="col-md-6">
						<ul class="user_quick_links">
							...
							$out .= '<li><a href="'.esc_url(add_query_arg("u", esc_attr($user_login->ID),get_page_link(vpanel_options('i_follow_user_page')))).'"><i class="icon-user-md"></i>'.__("Authors I Follow","vbegy").'</a></li>
					...
					<div class="col-md-6">
						<ul class="user_quick_links">
							...
							<li><a href="'.esc_url(add_query_arg("u", esc_attr($user_login->ID),get_page_link(vpanel_options('follow_question_page')))).'"><i class="icon-question-sign"></i>'.__("Follow questions","vbegy").'</a></li>
	...
	if (is_user_logged_in()) :
		...
		$out .= '<div class="ask_form inputs">
			<form class="login-form ask_login" action="'.home_url('/').'" method="post">
				...
				<input type="hidden" name="redirect_to" value="'.strip_tags( $protocol.'://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']).'">
		...
		</div>'.do_action('oa_social_login').(shortcode_exists('wordpress_social_login')?'<div class="clearfix"></div><br>'.do_shortcode("[wordpress_social_login]"):"");
	endif;
	return $out;
}
				...
				<div class="form-inputs clearfix">
					...
					if ($names_register == 1) {
						...
						<p>
							...
							<input name="first_name" id="first_name_'.$rand_w.'" type="text" value="'.$user_info->first_name.'">
						</p>
						<p>
							<label for="last_name_'.$rand_w.'" '.($names_required == 1?'class="required"':'').'>'.__("Last Name","vbegy").($names_required == 1?'<span>*</span>':'').'</label>
							<input name="last_name" id="last_name_'.$rand_w.'" type="text" value="'.$user_info->last_name.'">
						</p>
						<p>
							<label for="display_name_'.$rand_w.'" '.($names_required == 1?'class="required"':'').'>'.__("Display name","vbegy").($names_required == 1?'<span>*</span>':'').'</label>
							<input name="display_name" id="display_name_'.$rand_w.'" type="text" value="'.$user_info->display_name.'">
						</p>';
					}
					...
					if ($country_register == 1) {
						$out .= '<p>
							...
							<input type="text" '.($country_required == 1?'class="required-item"':'').' name="country" id="country_'.$rand_w.'" value="'.(isset($posted["country"])?$posted["country"]:"").'">
						</p>';
					}
					if ($age_register == 1) {
						$out .= '<p>
					...
					if ($terms_avtive_register == 1) {
						$out .= '<p class="question_poll_p">
							<label for="agree_terms-'.$rand.'" class="required">'.__("Terms","vbegy").'<span>*</span></label>
							<input type="checkbox" id="agree_terms-'.$rand.'" name="agree_terms" value="1" '.(isset($posted['agree_terms']) && $posted['agree_terms'] == 1?"checked='checked'":"").'>
